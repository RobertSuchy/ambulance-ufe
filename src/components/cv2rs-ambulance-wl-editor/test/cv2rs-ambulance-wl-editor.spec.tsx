...
import { Cv2rsAmbulanceWlEditor } from '../cv2rs-ambulance-wl-editor';
...
describe('cv2rs-ambulance-wl-editor', () => {
  ...
  it('buttons shall be of different type', async () => {
    ...
    const page = await newSpecPage({
      components: [Cv2rsAmbulanceWlEditor],
      html: `<cv2rs-ambulance-wl-editor entry-id="test-entry"
        ambulance-id="test-ambulance" api-base="http://sample.test/api">
      </cv2rs-ambulance-wl-editor>`,
    });
    await delay(300);
  ...
  it('first text field is patient name', async () => {
    ...
    const page = await newSpecPage({
       components: [Cv2rsAmbulanceWlEditor],
       html: `<cv2rs-ambulance-wl-editor entry-id="test-entry" ambulance-id="test-ambulance" api-base="http://sample.test/api"></cv2rs-ambulance-wl-editor>`,
    });
